 ...
 */
@Controller
@RequestMapping("/api/serve/debugeq/")
public class EquipmentDebugApiController {
    private static final Logger logger = Logger.getLogger(EquipmentDebugApiController.class);
    @Autowired
    ...
    public JSONObject list(String body) {
        ...
        try {
            ...
            if (StringUtil.isEmpty(param.getUserKey())) {
                throw new Exception("用户Key为空，不能查询设备调试记录数据");
            }
            //根据用户Key查询用户信息
            SysUser user = userService.findUserByUserKey(param.getUserKey());
            if (user == null) {
                throw new Exception("用户Key无效，不能查询设备调试记录数据");
            }
            if (param.getEqId() == null) {
                throw new Exception("设备Id为空，不能查询设备调试记录数据");
            }
            //查询数据总量
            Integer totalCount = serveDebugService.queryDebugCountByPage(param);
            if (totalCount > 0) {
                //查询数据
                List<TbServeDebug> list = serveDebugService.queryDebugListByPage(param);///customerService.queryCustomerListByPage(param);
                if (list != null && list.size() > 0) {
                    for (TbServeDebug maintenance : list) {
    ...
    public JSONObject delete(String body) {
        ...
        try {
            ...
            serveDebugService.deleteById(param.getId(), user.getUserId());
            result.setSuccess(Boolean.TRUE);
            result.setCode(0);
            result.setMessage("删除成功");
        } catch (Exception e) {
            logger.error("删除调试信息失败，单据Id【" + body + "】：", e);
    ...
    public JSONObject findById(String body) {
        ...
        try {
            ...
            if (param.getId() == null) {
                throw new Exception("单据Id为空，不能查询设备调试记录数据");
            }
            TbServeDebug debug = serveDebugService.findById(param.getId());
            resultData.setObj(debug);
            resultData.setSuccess(Boolean.TRUE);
            resultData.setCode(0);
            resultData.setMessage("查询成功");
        } catch (Exception e) {
            logger.error("查询调试单据详情失败!", e);
    ...
    public JSONObject save(String body) {
        ...
        try {
            //封装查询参数
            TbServeDebug param = (TbServeDebug) JSONObject.toBean(JSONObject.fromObject(body), TbServeDebug.class);
            if (StringUtil.isEmpty(param.getUserKey())) {
            ...
            serveDebugService.saveServeDebug(param, user.getUserId());
            resultData.setSuccess(Boolean.TRUE);
            resultData.setCode(0);
    ...
    public JSONObject edit(String body) {
        ...
        try {
            ...
            serveDebugService.editServeDebug(param, user.getUserId());
            resultData.setSuccess(Boolean.TRUE);
            resultData.setCode(0);
